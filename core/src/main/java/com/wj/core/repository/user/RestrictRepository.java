...
public interface RestrictRepository extends JpaRepository<SysRestrict, Integer> {
     ...
     * @return SysScreen
     */
    @Query(value = "select * from sys_restrict where create_date >= ?1 and create_date <= ?2", nativeQuery = true)
    public SysRestrict findByDate(String startDate, String endDate);
