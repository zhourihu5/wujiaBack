...
public interface BaseFloorRepository extends JpaRepository<BaseFloor, Integer> {
    ...
    @Query(value = "select count(id) from base_floor where communtity_id = ?1", nativeQuery = true)
    public Integer findCountByCommuntityId(Integer communtityId);

    @Query(value = "select count(id) from base_floor where issue_id = ?1", nativeQuery = true)
    public Integer findCountByIssueId(Integer issueId);

    @Query(value = "select count(id) from base_floor where district_id = ?1", nativeQuery = true)
    public Integer findCountByDistrictId(Integer districtId);

    BaseFloor findByCode(String code);

    List<BaseFloor> findByCodeLike(String code);
