...
public class DateUtil {
    ...
    public static int dateDiff(Date date1, Date date2, Pattern p) {
        ...
        switch (p) {
            ...
            case hour: {
                //计算区间天数
                Period period = new Period(begin, end, PeriodType.hours());
    ...
    public static int getMonthOfYear(@NotNull final Date date) {
        return get(date, Calendar.MONTH) + 1;
    }

    public static int getHourOfDay(@NotNull final Date date) {
        return get(date, Calendar.HOUR_OF_DAY);
    }

    public static int getMinuteOfDay(@NotNull final Date date) {
